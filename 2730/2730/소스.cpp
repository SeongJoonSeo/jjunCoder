...
bool isPrior(int d_m, int d_d, int s_m, int s_d, int y) {
	...
	if (leap && (s_d+7>29) && d_m==3 && s_m==2 && ((s_d + 7) % 29 >= d_d)){
		diff = 29+(d_d-s_d);
		return true;
	}
	//윤년아닐때
	if (!leap && (s_d+7>28) && d_m == 3 && s_m == 2 && ((s_d + 7) % 28 >= d_d)){
		diff = 28+(d_d-s_d);
		return true;
	}
	if (s_m == 1 || s_m == 3 || s_m == 5 || s_m == 7 || s_m == 8 || s_m == 10) {
		//이 제출 월 들은 다 31일까지 있다.
		if (s_m + 1 == d_m && (s_d+7>31) && ((s_d + 7) % 31 >= d_d)){
			diff = 31+(d_d-s_d);
			return true;
		}
	}
	if (s_m == 4 || s_m == 6 || s_m == 9 || s_m == 11) {
		...
		if (s_m + 1 == d_m && (s_d+7>30) && ((s_d + 7) % 30 >= d_d)){
			diff = 30+(d_d-s_d);
			return true;
		}
	}
...
bool isAfter(int d_m, int d_d, int s_m, int s_d, int y) {
	...
	if (leap && s_m == 3 && d_m == 2 && (d_d+7>29) && ((d_d + 7) % 29 >= s_d)){
		diff = 29+(s_d-d_d);
		return true;
	}
	//윤년아닐때
	if (!leap && s_m == 3 && d_m == 2 && (d_d+7>28) && ((d_d + 7) % 28 >= s_d)){
		diff = 28+(s_d-d_d);
		return true;
	}
	if (d_m == 1 || d_m == 3 || d_m == 5 || d_m == 7 || d_m == 8 || d_m == 10) {
		//이 제출 월 들은 다 31일까지 있다.
		if (d_m + 1 == s_m && (d_d+7>31) && ((d_d + 7) % 31 >= s_d)){
			diff = 31+(s_d-d_d);
			return true;
		}
	}
	if (d_m == 4 || d_m == 6 || d_m == 9 || d_m == 11) {
		//이 제출 월 들은 다 30일까지 있다.
		if (d_m + 1 == s_m && (d_d+7>30) && ((d_d + 7) % 30 >= s_d)){
			diff = 30+(s_d-d_d);
			return true;
		}
	}
